...
function NuevaSeccion() {
  return { competencia: '', descripcion: '', actitudes: [''] };
}
...
const CrearPauta = () => {
  ...
  const agregarAspecto = () => {
    if (aspectos.length >= MAX_ASPECTOS) return;
  ...
  const agregarActitud = (index) => {
    setAspectos(prev => prev.map((a, i) => {
      if (i !== index) return a;
      if (a.actitudes.length >= MAX_ACTITUDES) return a;
      return { ...a, actitudes: [...a.actitudes, ''] };
    }));
  };

  const actualizarActitud = (aspectIndex, actIndex, value) => {
    setAspectos(prev => prev.map((a, i) => {
      if (i !== aspectIndex) return a;
      const newActs = a.actitudes.map((act, j) => j === actIndex ? value : act);
      return { ...a, actitudes: newActs };
    }));
  };

  const eliminarActitud = (aspectIndex, actIndex) => {
    setAspectos(prev => prev.map((a, i) => {
      if (i !== aspectIndex) return a;
      if (a.actitudes.length === 1) return a; // siempre al menos 1
      const newActs = a.actitudes.filter((_, j) => j !== actIndex);
      return { ...a, actitudes: newActs };
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const aspectos_a_evaluar = aspectos.map(a => ({
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={agregarAspecto} disabled={aspectos.length >= MAX_ASPECTOS} className="text-sm text-blue-600">
                Agregar sección ({aspectos.length}/{MAX_ASPECTOS})
              </button>
            </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="md:col-span-1">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="pt-2">
                          <button type="button" disabled={a.actitudes.length >= MAX_ACTITUDES} onClick={() => agregarActitud(i)} className="text-sm text-blue-600">
                            Agregar actitud ({a.actitudes.length}/{MAX_ACTITUDES})
                          </button>
                        </div>
                      </div>
                    </div>
