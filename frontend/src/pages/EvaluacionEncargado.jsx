...
const EvaluacionEncargado = () => {
    ...
    // URL base del backend para servir archivos estáticos
    const API_URL = "http://localhost:3000";

    // Construye una URL válida para abrir un archivo en el navegador
    const buildFileUrl = (pathOrUrl) => {
        ...
        // Si ya viene como ruta absoluta tipo /uploads/xxx.pdf
        if (typeof pathOrUrl === "string" && pathOrUrl.startsWith("/")) {
            return `${API_URL}${pathOrUrl}`;
        }

        // Si viene como nombre de archivo, asumimos que está en /uploads
    ...
    useEffect(() => {
        const cargarDatos = async () => {
            try {
                ...
                // Ojo: en tu service a veces retornas data directo, otras veces { data: ... }
                const payload = res?.data ?? res;

                setDetalle(payload);
                console.log("DETALLE PRACTICA:", payload);
                console.log("BITACORAS:", payload?.bitacoras, "tipo:", typeof payload?.bitacoras);
            } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            Swal.fire("Listo", "Evaluación registrada correctamente.", "success");
            navigate("/dashboard");
        } catch (err) {
    ...
    const bitacoras = Array.isArray(detalle?.bitacoras)
        ? detalle.bitacoras
        : detalle?.bitacoras
            ? [detalle.bitacoras]
            : [];
            console.log("BITACORAS FRONT:", bitacoras);
        ...
        <div className="max-w-4xl mx-auto p-6 bg-gray-50 min-h-screen">
            <header className="mb-8">
                ...
                <p className="text-gray-600">
                    Califique el desempeño del estudiante basándose en los documentos entregados.
                </p>
            </header>
                                ...
                                {bitacoras.map((b, idx) => {
                                    ...
                                    return (
                                        <a
                                            ...
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="inline-flex items-center justify-center w-full max-w-xs px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors shadow-sm"
                                        >
                                            Ver Documento PDF (Bitácora {idx + 1})
                                        </a>
                                    );
                                })}
                            </div>
            ...
            <section className="bg-white shadow-lg rounded-xl p-6 border border-gray-100">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
                        <label className="block text-sm font-bold text-gray-700 mb-2">
                            Pauta de Corrección Firmada (PDF)
                        </label>
                        <input
                            type="file"
                        ...
                        <p className="text-xs text-gray-500 mt-2 italic">
                            Suba el archivo PDF que respalda la evaluación final.
                        </p>
                    </div>
